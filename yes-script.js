...
function initChapterTwoUnlock(music) {
    const photo = document.querySelector('.couple-photo')
    if (!photo) return
    ...
    const accessKey = 'sofia-chapter-two-access-v1'
    ...
    const showTapHint = (message) => {
        clearTapHint()
        const hint = document.createElement('div')
        hint.className = 'photo-secret-hint'
        hint.setAttribute('role', 'status')
        ...
        state.hintTimer = window.setTimeout(() => {
            hint.classList.remove('is-visible')
            window.setTimeout(clearTapHint, 220)
        }, 1500)
    }

    const showPhotoHeart = (event) => {
        const photoRect = photo.getBoundingClientRect()
        const hasPointerPosition = Number.isFinite(event?.clientX) && event.clientX > 0
        const heart = document.createElement('span')
        const x = hasPointerPosition
            ? event.clientX
            : photoRect.left + photoRect.width * (0.2 + Math.random() * 0.6)
        const y = hasPointerPosition
            ? event.clientY
            : photoRect.top + photoRect.height * (0.25 + Math.random() * 0.5)

        heart.className = 'photo-secret-heart'
        heart.textContent = state.tapCount % 3 === 0 ? '💕' : '💗'
        heart.style.left = `${x}px`
        heart.style.top = `${y}px`
        heart.style.setProperty('--heart-drift', `${-18 + Math.random() * 36}px`)
        heart.addEventListener('animationend', () => heart.remove(), { once: true })
    ...
    const closePasswordGate = () => {
        if (!state.overlay || state.unlocking) return
        const overlay = state.overlay
        state.overlay = null
        photo.setAttribute('aria-expanded', 'false')
        document.body.classList.remove('is-chapter-gate-open')
        overlay.classList.remove('is-visible')
        window.setTimeout(() => overlay.remove(), 330)
        state.previousFocus?.focus?.({ preventScroll: true })
    }

    const beginUnlock = (overlay) => {
        ...
        const card = overlay.querySelector('.chapter-unlock-card')
        card.removeAttribute('role')
        card.removeAttribute('aria-modal')
        card.removeAttribute('aria-labelledby')
        card.removeAttribute('aria-describedby')
        card.innerHTML = `
                <span class="chapter-unlock-kicker">Secreto encontrado</span>
                <img src="assets/walking-dachshund-idle-wag.webp?v=1" alt="Mía moviendo la cola">
                <h2>Capítulo 2 desbloqueado</h2>
                <p>Mía tiene una misión para ti…</p>
        `
        ...
        window.setTimeout(() => {
            try {
                ...
                // The surprise still works when storage is unavailable.
            }
            window.location.assign('dos-meses/#acceso-concedido')
        }, reduceMotion ? 550 : 1900)
    }

    const openPasswordGate = (event) => {
        if (state.unlocking) return

        if (state.overlay) {
            state.overlay.querySelector('#chapter-password')?.focus({ preventScroll: true })
            return
        }

        state.previousFocus = document.activeElement
        const overlay = document.createElement('div')
        overlay.className = 'chapter-unlock-overlay chapter-password-overlay'
        overlay.innerHTML = `
            <div class="chapter-unlock-card chapter-password-card" role="dialog" aria-modal="true" aria-labelledby="chapter-password-title" aria-describedby="chapter-password-copy chapter-password-hint">
                <button class="chapter-password-close" type="button" aria-label="Cerrar acceso al capítulo 2">×</button>
                <span class="chapter-unlock-kicker">Capítulo 2</span>
                <span class="chapter-password-heart" aria-hidden="true">💗</span>
                <h2 id="chapter-password-title">Solo para ti</h2>
                <p id="chapter-password-copy">Escribe la palabra secreta para continuar.</p>
                <form class="chapter-password-form" novalidate>
                    <label for="chapter-password">Contraseña</label>
                    <input id="chapter-password" name="chapter-password" type="password" autocomplete="off" autocapitalize="none" spellcheck="false" aria-describedby="chapter-password-hint chapter-password-error" required>
                    <p id="chapter-password-hint" class="chapter-password-hint"><span aria-hidden="true">💡</span> Pista: Lo más importante para mí</p>
                    <p id="chapter-password-error" class="chapter-password-error" role="alert" hidden>Esa no es. Piensa en la pista 💗</p>
                    <button class="chapter-password-submit" type="submit">Entrar al capítulo 2 <span aria-hidden="true">♥</span></button>
                </form>
            </div>
        `

        state.overlay = overlay
        document.body.classList.add('is-chapter-gate-open')
        photo.setAttribute('aria-expanded', 'true')
        document.body.appendChild(overlay)

        const card = overlay.querySelector('.chapter-password-card')
        const form = overlay.querySelector('.chapter-password-form')
        const input = overlay.querySelector('#chapter-password')
        const error = overlay.querySelector('#chapter-password-error')
        const closeButton = overlay.querySelector('.chapter-password-close')

        const submitPassword = (submitEvent) => {
            ...
            try {
                ...
                // The handoff hash keeps local-file previews working without storage.
            }
        ...
        closeButton.addEventListener('click', closePasswordGate)
        overlay.addEventListener('click', (overlayEvent) => {
            if (overlayEvent.target === overlay) closePasswordGate()
        })
        overlay.addEventListener('keydown', (keyEvent) => {
            ...
            if (keyEvent.key === 'Tab') {
                const focusable = [closeButton, input, overlay.querySelector('.chapter-password-submit')]
    ...
    const registerTap = (event) => {
        if (state.unlocking) return
        state.tapCount += 1
        showPhotoHeart(event)

        if (state.tapCount >= requiredTaps) {
            clearTapHint()
            openPasswordGate(event)
            return
        }

        if (state.tapCount === 5) {
            showTapHint('Esta foto guarda algo… 💗')
        } else if (state.tapCount === 10) {
            showTapHint('Mía dice que sigas… 👀')
        } else if (state.tapCount === 13) {
            showTapHint('Una más, ratita…')
        }
    }

    photo.setAttribute('aria-haspopup', 'dialog')
    photo.setAttribute('aria-expanded', 'false')
    photo.addEventListener('click', registerTap)
    ...
    if (window.location.hash === '#capitulo-2') {
        window.history.replaceState(null, '', `${window.location.pathname}${window.location.search}`)
        window.requestAnimationFrame(() => openPasswordGate())
    }
}
